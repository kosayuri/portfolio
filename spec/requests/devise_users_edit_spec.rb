...
RSpec.describe "devise_edit", type: :request do
  ...
  let(:user_edit) do
    attributes_for(:user, avatar: avatar_file, name: "edittest", email: "test@edit", comment: "commentedit", password: "passwordedit", password_confirmation: "passwordedit",
                          current_password: "testuser")
  end
  let(:bad_current_password_user_edit) do
    attributes_for(:user, avatar: avatar_file, name: "edittest", email: "test@edit", comment: "commentedit", password: "passwordedit", password_confirmation: "passwordedit", current_password: "")
  end
  ...
  describe 'ユーザー編集のコントローラテスト' do
    context '確認用パスワードを入力した場合' do
      ...
      it 'passwordがpasswordeditに変更すること' do
        expect(User.first.valid_password?("passwordedit")).to be true
      end
    end
    ...
    context '確認用パスワードを入力しなかった場合' do
      ...
      it 'avatarがedittest.pngに変更しないこと' do
        expect(User.first.avatar.identifier).not_to eq "edittest.png"
      end

      it 'nameがedittestに変更しないこと' do
        expect(User.first.name).not_to eq "edittest"
      end

      it 'emailがtest@editに変更しないこと' do
        expect(User.first.email).not_to eq "test@edit"
      end

      it 'commentがcommenteditに変更しないこと' do
        expect(User.first.comment).not_to eq "commentedit"
      end

      it 'passwordがpasswordeditに変更しないこと' do
        expect(User.first.valid_password?("passwordedit")).not_to be true
      end
    end
  end
end
